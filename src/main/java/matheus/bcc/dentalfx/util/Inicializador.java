...
public class Inicializador {
    public static boolean verificarConexao() {
        if (!SingletonDB.conectar()) {
            if (Confirmacao.exibir("Erro ao conectar com o banco.\nDeseja criar a base de dados?"))
                if (SingletonDB.criarBD(Constantes.NOME_BANCO)) {
                    if (!SingletonDB.criarTabelas(Constantes.SCRIPT_TABELAS, Constantes.NOME_BANCO)) {
                        ...
                        Erro.exibir("Erro ao criar as tabelas. Base de dados excluída.");
                    }
                } else
                    Erro.exibir("Erro ao criar a base de dados\n" + SingletonDB.getConexao().getMensagemErro());
            Platform.exit();
            return false;
        }
        return true;
